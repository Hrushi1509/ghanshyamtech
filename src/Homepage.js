...
const Homepage = () => {
  ...
  const fetchData = () => {
    ...
    setTasks(storedTasks.filter((task) => task.user === name));
  };
  useEffect(() => {
    fetchData();
  }, [modal]);
  ...
  const logout = () => {
    ...
    if (userData) {
      const currentUser = userData.find((user) => user.email.split('@')[0] === name)
  ...
  return (
    <div className="App">
      <div className='d-flex justify-content-between'>
        ...
        <div className='my-2 mx-2'><button onClick={logout} className='btn btn-danger'>Logout</button></div>
      </div>
